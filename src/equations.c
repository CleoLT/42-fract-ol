...
static t_number	mandel_reverse(t_number c, t_number z)
{
	...
	//z = z³ + 1 / c²
	c_divisor = c.x * c.x * c.x * c.x + 2 * c.x * c.x * c.y * c.y + c.y * c.y * c.y * c.y;
	result.x = z.x * z.x * z.x - 3 * z.x * z.y * z.y + (c.x * c.x + c.y * c.y) / c_divisor;
	result.y = 3 * z.x * z.x * z.y - z.y * z.y * z.y - 2 * c.x * c.y / c_divisor;
	return (result);
}
